...
class Ventana_Lista_Canciones(QWidget):
    ...
    def mostrar_canciones(self, canciones):
        ...
        for cancion in canciones:
            ...
            texto_interpretes = QLineEdit(
                cancion.get("compositor", cancion["compositor"])
            )
            ...
            self.caja_canciones.layout().addWidget(
                texto_duracion, fila, 2, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
            )

            boton_ver = QPushButton("Ver")
            boton_ver.setFixedSize(50, 25)
            boton_ver.clicked.connect(
                lambda estado, x=cancion["id"]: self.ver_cancion(x)
            )

            boton_borrar = QPushButton("Borrar")
            boton_borrar.setFixedSize(50, 25)
            boton_borrar.clicked.connect(
                lambda estado, x=cancion["id"]: self.eliminar_cancion(x)
            )
