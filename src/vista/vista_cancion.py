...
class Ventana_Cancion(QWidget):
    ...
    def inicializar_ventana(self):
        ...
        self.boton_adicionar.clicked.connect(
            lambda: self.mostrar_dialogo_crear_interprete()
        )
        ...
        self.caja_interpretes.layout().addWidget(
            etiqueta_nombre_interprete, 0, 0, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
        )

        etiqueta_acciones = QLabel("Acciones")
        etiqueta_acciones.setFont(QFont("Times", weight=QFont.Bold))
        self.caja_interpretes.layout().addWidget(
            etiqueta_acciones, 0, 1, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
        )
        ...
        self.distr_cancion.addWidget(
            etiqueta_interpretes, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
        )
    ...
    def mostrar_cancion(self, cancion=None, interpretes=None):
        ...
        self.texto_cancion.setText(
            cancion.get("titulo", "") if cancion is not None else ""
        )
        self.texto_minutos.setText(
            str(cancion.get("minutos", "")) if cancion is not None else ""
        )
        self.texto_segundos.setText(
            str(cancion.get("segundos", "")) if cancion is not None else ""
        )
        self.texto_compositor.setText(
            cancion.get("compositor", "") if cancion is not None else ""
        )
    ...
    def mostrar_interpretes(self, interpretes):
        ...
        for i, interprete in enumerate(interpretes):
            ...
            self.caja_interpretes.layout().addWidget(
                campo_nombre, fila, 0, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
            )
            boton_ver = QPushButton("Ver")
            boton_ver.setFixedSize(50, 25)
            boton_ver.clicked.connect(
                lambda estado, n_interprete=i: self.mostrar_dialogo_crear_interprete(
                    n_interprete
                )
            )
            widget_botones.layout().addWidget(
                boton_ver, fila, 0, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
            )
            boton_borrar = QPushButton("Borrar")
            boton_borrar.setFixedWidth(50)
            boton_borrar.setFixedSize(50, 25)
            boton_borrar.clicked.connect(
                lambda estado, n_interprete=i: self.eliminar_interprete(n_interprete)
            )
            widget_botones.layout().addWidget(
                boton_borrar, fila, 1, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
            )

            widget_botones.layout().setContentsMargins(0, 0, 0, 0)

            self.caja_interpretes.layout().addWidget(
                widget_botones, fila, 1, QtCore.Qt.AlignTop | QtCore.Qt.AlignCenter
            )
            fila += 1
        ...
        if len(self.interpretes) == 0:
            ...
            if self.cancion_actual == None:
                ...
                if (
                    ...
                    or self.cancion_actual["minutos"] == ""
                    or self.cancion_actual["segundos"] == ""
                ):
                    mensaje_error = QMessageBox()
                    mensaje_error.setIcon(QMessageBox.Critical)
                    mensaje_error.setWindowTitle("Error al guardar canción")
                    ...
                    int(self.cancion_actual["minutos"]) == 0
                    and int(self.cancion_actual["segundos"]) < 10
                ):
                    mensaje_error = QMessageBox()
                    mensaje_error.setIcon(QMessageBox.Critical)
                    mensaje_error.setWindowTitle("Error al guardar canción")
    ...
    def mostrar_dialogo_crear_interprete(self, n_interprete=-1):
        ...
        if n_interprete != -1:
            ...
            txt2.setPlainText(
                self.interpretes[n_interprete].get("texto_curiosidades", "")
            )
        ...
        if n_interprete == -1:
            ...
            butAceptar.clicked.connect(
                lambda: self.agregar_interprete(txt1.text(), txt2.toPlainText())
            )
        else:
            self.dialogo_nuevo_interprete.setWindowTitle("Modificar interprete")
            butAceptar.clicked.connect(
                lambda: self.modificar_interprete(
                    n_interprete, txt1.text(), txt2.toPlainText()
                )
            )
    ...
    def agregar_interprete(self, nombre, texto_curiosidades):
        ...
        if nombre == "" or texto_curiosidades == "":
            ...
            self.interpretes.append(
                {"id": "n", "nombre": nombre, "texto_curiosidades": texto_curiosidades}
            )
            self.mostrar_interpretes(self.interpretes)
