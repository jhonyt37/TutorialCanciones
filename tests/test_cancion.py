...
class CancionTestCase(unittest.TestCase):
    ...
    def test_cancion_sin_interpretes(self):
        ...
        cancion = self.coleccion.agregar_cancion(
            titulo_cancion,
            minutos_cancion,
            segundos_cancion,
            compositor_cancion,
    ...
    def test_cancion_varios_interpretes(self):
        ...
        consulta = (
            self.session.query(Cancion).filter(Cancion.titulo == titulo_cancion).first()
        )
    ...
    def test_cancion_con_album(self):
        ...
        self.coleccion.agregar_interprete(
            "Alejandra Guzman", "Canción dedicada a su ...", -1
        )
        minutos_cancion = self.data_factory.pyint(0, 60)
        segundos_cancion = self.data_factory.pyint(0, 60)
        compositor_cancion = self.data_factory.name()
        ...
        consulta2 = (
            self.session.query(Cancion).filter(Cancion.titulo == "Bye mamá").first()
        )
        self.assertNotEqual(len(consulta2.albumes), 1)
    ...
    def test_cancion_repetida_album(self):
        ...
        cancion = self.coleccion.agregar_cancion(
            ...
            compositor_cancion2,
            consulta1,
            [{"nombre": nombre_interprete2, "texto_curiosidades": texto_curiosidades2}],
        )
        self.assertEqual(cancion, False)

    def test_editar_cancion(self):
        consulta1 = (
            self.session.query(Cancion).filter(Cancion.id == 2).first().compositor
        )
        consulta2 = (
            self.session.query(Interprete)
            .filter(Interprete.nombre == "Franco de Vita")
            .first()
        )
        ...
        if consulta2 is None:
            ...
            self.coleccion.editar_cancion(
                2,
                "Bye mamá",
                minutos_cancion,
                segundos_cancion,
                "J.R.Florez y Difelisatti",
                [
                    {
                        "id": "2",
                        "nombre": "Alejandra Guzman",
    ...
    def test_buscar_canciones_por_titulo(self):
        ...
        self.coleccion.agregar_album(
            titulo_cancion, anio_cancion, descripcion_cancion, "CD"
        )
        consulta1 = (
            self.session.query(Album).filter(Album.titulo == titulo_cancion).first().id
        )
        ...
        self.coleccion.agregar_cancion(
            ...
            compositor_cancion,
            consulta1,
            [{"nombre": nombre_interprete, "texto_curiosidades": texto_curiosidades}],
        )
